...
for k=1:length(Datapath)
    ...
    for probe=1:1
        for shank=1:1
            ...
            % 修改chMap使序号从1开始
            chMap = readNPY(fullfile(datapath, 'channel_map.npy'))-15;  % Order in which data was streamed to disk; must be 1-indexed for Matlab
            ...
            if ~isempty(cids_good)
                % 修改channel_good使序号从1开始
                channel_good=channel(find(ismember(cids,cids_good))) - 16;%already 1-384 range
...
for probe=1:1
    for i_session = 1:size(Datapath,1)
        single_unit_dir = [Datapath{i_session},'Kilosort_SingleUnits',num2str(probe),'\'];
...
for probe=1:1
    for i_session = 1:size(Datapath,1)
        single_unit_dir = [Datapath{i_session},'Kilosort_SingleUnits',num2str(probe),'\'];%num2str(position),
...
function wf = getWaveForms(gwfparams)
    ...
    chMap = readNPY(fullfile(gwfparams.dataDir, 'channel_map.npy'))-15; % Order in which data was streamed to disk; must be 1-indexed for Matlab
