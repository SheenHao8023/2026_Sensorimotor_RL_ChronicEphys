...
%     end
    ...
    methods (Access = public)
        function obj = YesNoDiscriminationCIBRData(fileName, trimData)
            %
            ...
            if iscell(fileName)
                ...
                for i_solo_file = 1:length(fileName)
                    if i_solo_file == 1
                        obj = Solo.YesNoDiscriminationCIBRData(fileName{i_solo_file});
                    else
                        obj_tmp = Solo.YesNoDiscriminationCIBRData(fileName{i_solo_file});
            ...
            elseif isstr(fileName)
                ...
                obj.mouseName = fileName(1:(i_str(1)-5));
                ...
                Outcomes = []; EarlyLicks = [];
                for x = 1:SessionData.nTrials
                    if SessionData.TrialSettings(x).GUI.ProtocolType==3 | SessionData.TrialSettings(x).GUI.ProtocolType==4&isempty(findstr(obj.sessionName,'passive'))
                    ...
                    if SessionData.TrialSettings(x).GUI.ProtocolType==4&isempty(findstr(obj.sessionName,'passive'))
...
%                 obj.Trim_Trials(trimTrials);
        ...
        function  [Licking_Side, Licking_Timing, Response_Time]=Lick_SideTime( obj );
            ...
            for i_trial = 1:sum(obj.trials.trialNums(1))
                ...
                if isfield(eventHistory_iTrial.Events,'Port2In')
                    lick_data_tmp = [(eventHistory_iTrial.Events.Port2In)' (eventHistory_iTrial.Events.Port2In)'*0+'r'];
                end
        ...
        function load_Wavesurfer_Data(obj, wavesurfer_filename)
            ...
            TimeStamp = (1:size(wavesurfer_trig,1))/20000;
...
%             idx=find(i_trig_off-i_trig_on>28000);
...
%                 end
                ...
                if i_trig_off(1)-i_trig_on(1)<5*20000
                ...
                if i_trig_off(1)-i_trig_on(1)>5*20000
                if i_trig_off(i_rep)-i_trig_on(i_rep)<5*20000
...
%                     end
        ...
        function trial_NO = func_read_bitcode(obj, bitcode, time_stamp)
            ...
            bitcode(1) = 0;
            bitcode(end) = 0;
            i_bitcode_on = find(diff(bitcode>max(double(bitcode))*0.8)==1);
            i_bitcode_off = find(diff(bitcode>max(double(bitcode))*0.8)==-1);
            
            idx=find(bitcode>max(double(bitcode))*0.8);
...
%                 bit_state = mean(bitcode(i_sample));
                ...
                if bit_state>max(double(bitcode))*0.8
                    bit_tmp = [bit_tmp '1'];
                else
                    bit_tmp = [bit_tmp '0'];
                end
